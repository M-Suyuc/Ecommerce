...
export const cartSlice = createSlice({
  ...
  reducers: {
      ...
      if (findProduct) {
        const tempCart = state.productList.map(product => {
          if (product.id === id) {
            ...
            return {
              ...product, quantity: tempQty, totalPrice: tempTotalPrice
            }
          } else {
            return product
          }
    ...
    DeleteProduct: (state, action) => {
      const delProduct = state.productList.filter(item => item.id !== action.payload)
    ...
    getCartTotal: (state) => {
      state.totalAmount = state.productList.reduce((acumulador, currentvalue) => {
